...
export async function buildApp() {
  ...
  const BookingRow = T.Object({
    organizer: T.String({ minLength: 1 }),
    audienceId: T.String({ minLength: 1 }),
    startTime: T.String({ format: "date-time" }),
    endTime: T.String({ format: "date-time" }),
  });

  const BookingUpdate = T.Partial(BookingRow);
  ...
  app.get(
    "/api/bookings",
    {
      schema: {
        ...
        tags: ["Bookings"],
        summary: "Возвращает бронирования",
        description: "Получаем данные всех бронирований.",
        ...
        response: {
          200: {
            ...
            content: { "application/json": { schema: T.Array(BookingData) } },
          },
          429: {
            description: "Too Many Requests",
            headers: {
  ...
  app.get(
      ...
      schema: {
        ...
        response: {
          200: {
            ...
            content: { "application/json": { schema: BookingData } },
          },
          404: {
    ...
    async (req, reply) => {
      ...
      return {
        ...
        audienceId: booking.audienceId,
        startTime: booking.startTime.toISOString(),
        endTime: booking.endTime.toISOString(),
        ...(booking.audience
          ? { audience: { number: booking.audience.number } }
          : {}),
      };
    },
  );
  ...
  app.post("/api/dev/seed-rooms", { schema: { hide: true } }, async () => {
    ...
    const audiences = [
      { number: "01", name: "Аудитория 01" },
      { number: "02", name: "Аудитория 02" },
      { number: "03", name: "Аудитория 03" },
      ...
      schema: {
        ...
        response: {
          201: {
            description: "Создано",
            content: {
              "application/json": { schema: T.Object({ id: T.String() }) },
            },
          },
          400: {
  ...
  app.patch(
    ...
    async (req, reply) => {
      ...
      const nextStart = body.startTime
        ? parseDateOrNull(body.startTime)
        : existing.startTime;
      const nextEnd = body.endTime
        ? parseDateOrNull(body.endTime)
        : existing.endTime;
      ...
      if (body.organizer !== undefined) data.organizer = body.organizer.trim();
      if (body.audienceId !== undefined) data.audienceId = body.audienceId;
